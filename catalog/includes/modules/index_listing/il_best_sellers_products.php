...
*/
  ...
  class il_best_sellers_products {
    ...
    public function execute() {
      global $oscTemplate, $currencies, $current_category_id;

      if (isset($current_category_id) && ($current_category_id > 0)) {
        $best_sellers_products_query = tep_db_query("SELECT DISTINCT p.*, pd.*, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price from products p left join specials s on p.products_id = s.products_id left join products_description pd on  p.products_id = pd.products_id left join products_to_categories p2c on pd.products_id = p2c.products_id LEFT JOIN categories c on p2c.categories_id = c.categories_id where p.products_status = '1' and pd.language_id = '" . (int)$_SESSION['languages_id'] . "' and '" . (int)$current_category_id . "' in (c.categories_id, c.parent_id) order by p.products_ordered desc, pd.products_name limit " . (int)MODULE_INDEX_LISTING_BEST_SELLERS_PRODUCTS_MAX_DISPLAY_PRODUCTS);
      } else {
        $best_sellers_products_query = tep_db_query("SELECT DISTINCT p.*, pd.*, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price from products p left join specials s on p.products_id = s.products_id, products_description pd where p.products_status = '1' and p.products_id = pd.products_id and pd.language_id = '" . (int)$_SESSION['languages_id'] . "' order by p.products_ordered desc, pd.products_name limit " . (int)MODULE_INDEX_LISTING_BEST_SELLERS_PRODUCTS_MAX_DISPLAY_PRODUCTS);
      }
      ...
      if (tep_db_num_rows($best_sellers_products_query) > 0) {
        ...
        ob_start();
        include(DIR_WS_MODULES . $this->group . '/templates/best_sellers_products.php');
