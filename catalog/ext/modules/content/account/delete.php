...
if (isset($_GET['account'], $_GET['key']) && (!empty($_GET['account']) || !empty($_GET['key']))) {
  ...
  if ($error === false) {
    ...
    while ($orders = tep_db_fetch_array($orders_query)) {
      tep_db_query("update orders set customers_name = '" . tep_db_input($customer) . "', customers_street_address = '" . tep_db_input($customer) . "', customers_telephone = '+0123456789', customers_email_address = '" . tep_db_input($customer . '@' . $customer) . "', delivery_name = '" . tep_db_input($customer) . "', delivery_street_address = '" . tep_db_input($customer) . "', billing_name = '" . tep_db_input($customer) . "', billing_street_address = '" . tep_db_input($customer) . "' where orders_id = '" . (int)$orders['orders_id'] . "'");
    }
...
if ($account_delete_initiated === true) {
  ...
  <div class="col-lg-6 mb-5">
    <div class="form-check mb-3">
      <?php echo tep_draw_checkbox_field('delete', 'on', false, 'class="form-check-input" id="delete"'); ?>
      <label class="form-check-label" for="delete">
        <?php echo MODULE_CONTENT_ACCOUNT_DELETE_HEADING_TITLE; ?>
      </label>
    </div>
